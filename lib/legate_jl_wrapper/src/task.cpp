...
JULIA_LEGATE_UFI_EXPORT int legate_get_started_count() {
  return g_started_count.load();
}
...
inline void JuliaTaskInterface(legate::TaskContext context, bool is_gpu) {
  ...
  DEBUG_PRINT("JuliaTaskInterface(task=%u): started=%d, active=%d\n", task_id,
              g_started_count.load(), g_active_calls.load());

  DEBUG_PRINT("JuliaTaskInterface starting for task %u in slot search...\n",
              task_id);

  const size_t ni =
      std::min((size_t)context.num_inputs(), (size_t)MAX_UFI_ARGS);
  const size_t no =
      std::min((size_t)context.num_outputs(), (size_t)MAX_UFI_ARGS);
  const size_t nst = context.num_scalars();
  const size_t ns = std::min((nst > 1 ? nst - 1 : 0), (size_t)MAX_UFI_ARGS);

  if (context.num_inputs() > MAX_UFI_ARGS ||
      context.num_outputs() > MAX_UFI_ARGS ||
      (nst > 1 && nst - 1 > MAX_UFI_ARGS)) {
    ERROR_PRINT(
        "Task %u: Argument count exceeds MAX_UFI_ARGS (%d). Inputs: %zu, "
        "Outputs: %zu, Scalars: %zu\n",
        task_id, MAX_UFI_ARGS, context.num_inputs(), context.num_outputs(),
        nst - 1);
    abort();
  }
  ...
  // Wait for completion
  {
    ...
    slot.cv.wait_for(lock, std::chrono::seconds(300),
                     [&] { return slot.task_done.load(); });
...
void JuliaCustomTask::cpu_variant(legate::TaskContext context) {
  JuliaTaskInterface(context, false);
}
#if LEGATE_DEFINED(LEGATE_USE_CUDA)
void JuliaCustomGPUTask::gpu_variant(legate::TaskContext context) {
...
void wrap_ufi(jlcxx::Module& mod) {
  ...
  mod.method("legate_get_active_call_count",
             &ufi::legate_get_active_call_count);
  mod.method("legate_get_active_slot_count",
             &ufi::legate_get_active_slot_count);
